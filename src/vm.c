...
#define BINARY_OP(valueType, op) \
    ...
    for (;;) {
        ...
        int ipOffset = (int)(frame->ip - frame->function->chunk.code);
        ...
        printf("\n");
        disassembleInstruction(&frame->function->chunk, ipOffset);
#endif
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_LOOP: {
                ...
                if (newIp < frame->function->chunk.code ||
                    newIp >= frame->function->chunk.code + frame->function->chunk.count) {
                    runtimeError("Invalid loop jump offset: offset=%d, ip was %td",
                                 offset, (frame->ip - frame->function->chunk.code));
                    return INTERPRET_RUNTIME_ERROR;
                }
                frame->ip = newIp;
                break;
